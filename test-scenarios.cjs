...
const { defaultInputs, conservativeScenario, moderateScenario } = require('./client/src/lib/data.ts');
...
try {
  ...
  console.log(`  Peak Members: ${results1.kpis.peakMembers}`);
  console.log('');
  ...
  global.scenario1Results = results1;
} catch (error) {
...
try {
  ...
  // Sanity checks
  const checks2 = [];
  if (global.scenario1Results && results2.kpis.launchMRR < global.scenario1Results.kpis.launchMRR) {
    checks2.push('⚠️  4-physician MRR lower than 1-physician (unexpected)');
  }
  if (global.scenario1Results && results2.kpis.membersAtLaunch < global.scenario1Results.kpis.membersAtLaunch) {
...
try {
  ...
  // Sanity checks
  const checks3 = [];
  if (global.scenario1Results && results3.kpis.physicianROI > global.scenario1Results.kpis.physicianROI) {
...
console.log('SCENARIO 4: Churn Rate Sensitivity Test');
console.log('-'.repeat(80));
...
try {
  ...
  console.log('Checking: Do members grow month-over-month during projection?');
  console.log('');
...
console.log('   1. Review the numbers above with business stakeholders');
console.log('   2. Verify ROI calculations match expectations');
console.log('   3. Test edge cases (very high churn, zero growth, etc.)');
console.log('   4. Run User Acceptance Testing with physicians');
console.log('');
