  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <KPICard
          title="Cash Runway"
          value={typeof runway === 'number' ? `${runway} months` : runway}
          subtitle="Until cash depleted"
          icon={Calendar}
          formula={formulas.cashRunway}
          valueClassName="text-primary"
        />

        <KPICard
          title="Final Cash (M18)"
          value={`$${(finalCash / 1000).toFixed(0)}k`}
          subtitle="End of Month 18"
          icon={DollarSign}
          formula={formulas.finalCash}
          valueClassName={finalCash >= 0 ? 'text-green-600' : 'text-red-600'}
        />

        <KPICard
          title="Peak Cash"
          value={`$${(peakCash / 1000000).toFixed(2)}M`}
          subtitle="Highest cash position"
          icon={TrendingUp}
          formula={formulas.peakCash}
          valueClassName="text-primary"
        />

        <KPICard
          title="Avg Monthly Burn"
          value={`$${(avgMonthlyBurn / 1000).toFixed(0)}k`}
          subtitle="Across 19 months"
          icon={TrendingDown}
          formula={formulas.avgMonthlyBurn}
        />
      </div>
